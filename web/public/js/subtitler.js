...
//   // Logout button (added in subtitler.html)
...
/**
 * ROLE — Subtitler UI controller (browser) with session support
 */
  ...
  try {
    ...
    if (el.sessionInfo) {
      el.sessionInfo.innerHTML = `
        <div style="background:rgba(46,204,113,0.05);border:1px solid rgba(46,204,113,0.2);border-radius:4px;padding:10px;margin-bottom:12px;">
          <div style="font-size:0.75em;color:#666;margin-bottom:2px;">Session</div>
          <div style="font-weight:600;font-size:0.9em;">${STC.escapeHtml(session.id)}</div>
          <div style="font-size:0.8em;color:#888;">${STC.escapeHtml(session.title)}</div>
        </div>
      `;
    }
  } catch (e) {
  ...
  switch (msg.type) {
    ...
    case 'error':
      if (msg.error === 'AUTH_REQUIRED' || msg.error === 'INVALID_TOKEN') {
        ...
        localStorage.removeItem('stc_token');
        location.replace('/subtitler-entry.html');
      } else if (msg.error === 'SESSION_INVALID' || msg.error === 'SESSION_ENDED') {
        alert('Session invalide ou terminée.');
        localStorage.removeItem('stc_session_id');
        location.replace('/subtitler-entry.html');
      } else if (msg.error === 'WRONG_SESSION') {
        alert(msg.message || 'Mauvaise session.');
        localStorage.removeItem('stc_session_id');
        location.replace('/subtitler-entry.html');
      }
      break;
