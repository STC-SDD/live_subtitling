...
/**
 * ROLE — WebSocket realtime hub with session validation
 */
/**
 * ROLE — WebSocket realtime hub with optional session validation
 */

import { WebSocketServer } from 'ws';
import { state, log, isLiveRunning, getLiveTimestamp, config } from './core.js';
import * as services from './services.js';
...
function handleIdentify(ws, msg) {
  ...
  // Admin and spectator don't need token
  if (clientType !== 'subtitler') {
    ws.clientType = clientType;
  ...
  // Subtitler authentication (token required)
  if (!token || typeof token !== 'string') {
    services.send(ws, { type: 'error', error: 'AUTH_REQUIRED' });
    return;
  }
  ...
  try {
    ...
    // Session validation (OPTIONAL - can work without session)
    if (sessionId) {
      const session = sessionStore.findSessionById(sessionId);
      
      if (!session) {
        services.send(ws, { type: 'error', error: 'SESSION_INVALID' });
        return;
      }

      if (session.status === 'ended') {
        services.send(ws, { type: 'error', error: 'SESSION_ENDED' });
        return;
      }

      ws.sessionId = sessionId;
      log.info('WS', `Identified: subtitler (${ws.subtitlerName}) - Session: ${sessionId}`);
    } else {
      // Free mode - no session
      log.info('WS', `Identified: subtitler (${ws.subtitlerName}) - Free mode`);
    }

    // Auto-join fragment session
    if (!state.fragment.subtitlers.has(ws.odId)) {
      handleFragmentJoin(ws, { name: ws.subtitlerName });
    }
  } catch (e) {
...
function handleFragmentJoin(ws, msg) {
  ...
  // SESSION MODE: If there's an active session, only allow matching sessions
  if (state.currentSessionId) {
    // If subtitler has no session (free mode), warn but allow
    if (!ws.sessionId) {
      log.warn('WS', `Subtitler ${name} joining in free mode while session ${state.currentSessionId} is active`);
      // Allow joining but they might be removed if admin wants session-only
    } else if (ws.sessionId !== state.currentSessionId) {
      // Wrong session - reject
      log.warn('WS', `Subtitler ${name} tried to join with wrong session (has: ${ws.sessionId}, active: ${state.currentSessionId})`);
      services.send(ws, { 
        type: 'error', 
        error: 'WRONG_SESSION',
        message: `Cette session n'est pas active. Session active: ${state.currentSessionId}` 
      });
      return;
    }
  }
